...
module.exports.addCommande = async (req, res) => {
  try {
    const { model, prix, matricule, produit } = req.body;

    if (!model || !prix || !matricule) {
      return res.status(400).json({ message: "Données invalides, tous les champs sont requis." });
    }

    const newCommande = new commandeModel({ model, prix, matricule });
